...
public class expertReactionListener extends ListenerAdapter {
    public void onGuildMessageReactionAdd(GuildMessageReactionAddEvent event) {
        if (event.getChannel().getId().equals("588823110250266641")) {
    ...
    private static void executeAction(GuildMessageReactionAddEvent event, String mode, Message msg, List<User> users) {
        ...
        if (!mode.equals("remove")) {
            ...
            try {
                answer = core.databaseHandler.database(event.getGuild().getId(), "select * from quizquestions where status = " + msg.getId());
        ...
        switch (mode) {
            ...
            case "report":
                ...
                report.addField("Channel:", Objects.requireNonNull(event.getGuild().getTextChannelById(588823110250266641L)).getAsMention(), false);
                ...
                Objects.requireNonNull(event.getGuild().getTextChannelById(434007950852489216L)).sendMessage(report.build()).queue(message -> {
                    ...
                    try {
                        databaseHandler.database(event.getGuild().getId(), "insert into reports(report_id, victim_id, offender_id, channel, cause, info, ruling) " +
                                "values ('" + message.getId() + "', '" + finalVictim_id + "', '" + finalUser_question.getId() + "', '" + event.getGuild().getTextChannelById(588823110250266641L) +
