...
public class StatisticsListener extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(@NotNull GuildMessageReceivedEvent event) {
        String[] answer1 = null;
        try {
            answer1 = DatabaseHandler.database(event.getGuild().getId(), "select words, msg, chars from users where id = '" + event.getAuthor().getId() + "'");
        ...
        try {
            DatabaseHandler.database(event.getGuild().getId(), "update users set words = " + newWords + ", msg = " + newMsg + ", chars = " + newChars + " where id = '" + event.getAuthor().getId() + "'");
        } catch (SQLException ignored) {
        }
    }
}