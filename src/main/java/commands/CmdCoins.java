                ...
                case "gift":
                    if (Long.parseLong(coins_level[1]) >= 50) {
                        ...
                        try {
                            ...
                            member = GetUser.getMemberFromInput(list.toArray(new String[0]), event.getAuthor(), event.getGuild(), channel);
                        } catch (Exception e) {
                            e.printStackTrace();
                        ...
                        if (amount < 0) {
                            ...
                            try {
                                if (Long.parseLong(coins_level[0]) < amount) {
                                    ...
                                    DatabaseHandler.database(event.getGuild().getId(), "update users set coins = coins - " + amount + " where id = '" + event.getAuthor().getId() + "'");
